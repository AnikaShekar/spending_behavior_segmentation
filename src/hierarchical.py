...
# n_clusters=5 → same as K-Means for fair comparison
# linkage='ward' → merges clusters minimizing within-cluster variance
...
Silhouette Score     0.193    0.176
Winner               ✅      ❌
K-Means wins by 0.017 — small difference but K-Means is clearly better for this dataset.

Why K-Means won:
This dataset has globular (round shaped) clusters — exactly what K-Means is designed for. 
Agglomerative works better when clusters have irregular or hierarchical shapes.

Scatter Plot Comparison:
K-Means (left): Red cluster clearly separated at top
                Yellow spread far right
                Clean boundaries between groups

Agglomerative (right): Blue takes over a large area — over-merged
                       Red is tiny — under-represented
                       Boundaries are messier and less distinct

Cluster Size Comparison:
                    K-Means            Agglomerative
Largest cluster     3164               4093 🔴 too big
Smallest cluster    395                300
Distribution        More balanced      Unbalanced

Agglomerative created one cluster with 4093 customers — nearly half the dataset in one group.
That's poor segmentation for business use.


What to say in viva:
"We compared K-Means with Agglomerative Hierarchical Clustering using Silhouette Score as the evaluation metric.
K-Means achieved 0.193 vs Agglomerative's 0.176. 
K-Means also produced more balanced cluster sizes — 3164 vs 4093 for the largest cluster. 
Since the dataset contains globular clusters, K-Means is better suited and was selected as the final algorithm."
"""